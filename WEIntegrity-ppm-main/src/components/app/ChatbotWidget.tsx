...
function answer(workspace: "ppm" | "hr", q: string): string {
  ...
  if (workspace === "hr") {
    if (lower.includes("leave")) return "You have **14** of 21 annual leave days remaining. Last approved leave: Aug 12–14. Next holiday: Oct 2 (Gandhi Jayanti).";
    if (lower.includes("attendance")) return "Today's attendance: **86 present**, 4 on leave, 2 WFH, 1 late check-in. Team punctuality is up 3.2% week-over-week.";
    if (lower.includes("payroll")) return "Current payroll cycle closes on the 28th. 124 employees in scope; 3 pending approvals from finance.";
    if (lower.includes("recruit") || lower.includes("hiring")) return "Pipeline: 47 active candidates · 12 in interviews · 4 offers extended · 2 onboarding next week.";
    if (lower.includes("policy")) return "Policy snapshot: 21 PL + 8 SL + 10 CL per year. WFH up to 8 days/month. Maternity 26 weeks · Paternity 15 days.";
  ...
  if (lower.includes("sprint")) return "**Sprint 24.2** — Day 7/14. 38 of 64 SP completed (59%). Burndown is on trend, projected to close at 96% of committed scope.";
  if (lower.includes("risk") || lower.includes("at risk")) return "**At risk:** Mobile Re-platform (budget 92% spent, 12 days slip). **Watch:** Data Lake Migration (2 unresolved blockers).";
  if (lower.includes("velocity")) return "Trailing 6-sprint velocity: 52 · 58 · 61 · 55 · 64 · 67 SP. Mean 59.5, trending **+8.4%**. Capacity for Sprint 24.3 ≈ 70 SP.";
  if (lower.includes("block")) return "3 blocked tasks: YGN-481 (waiting on design), YGN-503 (infra approval), YGN-512 (3rd-party API outage).";
  if (lower.includes("deadline")) return "This week: 5 stories due, 1 epic milestone (Checkout v2 → Fri). 2 items at risk of slipping past due date.";
  if (lower.includes("workload") || lower.includes("team")) return "Workload: 4 engineers at healthy load, 2 over-allocated (Sofia, Daniel). Recommend rebalancing 6 SP to Aisha or Marcus.";
...
export function ChatbotWidget() {
  ...
  const send = (text: string) => {
    ...
    setTimeout(() => {
      setMessages((m) => [
        ...m,
        { id: crypto.randomUUID(), role: "assistant", content: answer(workspace, q), ts: Date.now() },
      ]);
      setTyping(false);
    }, 700 + Math.random() * 500);
  };
        ...
        {open && (
          <motion.div
            ...
            style={(() => {
              ...
              const left = Math.min(Math.max(margin, baseX - panelW + FAB_SIZE), vw - panelW - margin);
          ...
          >
            ...
            <div className="px-4 h-14 flex items-center justify-between border-b border-border bg-gradient-primary text-white shrink-0">
              <div className="flex items-center gap-2.5 min-w-0">
                ...
                <div className="min-w-0">
                  ...
                  <div className="text-[11px] text-white/70 capitalize">{workspace} workspace · online</div>
                </div>
              </div>
              <div className="flex items-center gap-1">
                <button onClick={() => setMin((v) => !v)} className="h-8 w-8 grid place-items-center rounded-md hover:bg-white/15 transition" aria-label="Minimize">
                  <Minus className="h-4 w-4" />
                </button>
                <button onClick={() => setOpen(false)} className="h-8 w-8 grid place-items-center rounded-md hover:bg-white/15 transition" aria-label="Close">
                  <X className="h-4 w-4" />
                </button>
              </div>
            </div>
            ...
            {!min && (
              <>
                {/* Messages */}
                <div ref={scrollRef} className="flex-1 overflow-y-auto scrollbar-thin px-4 py-4 space-y-3 bg-background/50">
                  {messages.map((m) => (
                    <div key={m.id} className={`flex gap-2.5 ${m.role === "user" ? "flex-row-reverse" : ""}`}>
                      <div className={`h-7 w-7 shrink-0 rounded-full grid place-items-center text-white ${m.role === "user" ? "bg-muted-foreground/70" : "bg-gradient-primary"}`}>
                  ...
                  {messages.length <= 1 && (
                    <div className="pt-2">
                      <div className="text-[11px] font-semibold uppercase tracking-wider text-muted-foreground mb-2">Suggested</div>
