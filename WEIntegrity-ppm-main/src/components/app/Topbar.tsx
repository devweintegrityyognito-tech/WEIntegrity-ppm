    ...
    <header className="sticky top-0 z-30 bg-background/80 backdrop-blur-md border-b border-border">
      <div className="flex items-center gap-4 px-6 h-14">
        ...
        <div className="flex-1 max-w-md ml-auto relative hidden md:block">
          ...
          />
          <kbd className="absolute right-2 top-1/2 -translate-y-1/2 text-[10px] font-mono px-1.5 py-0.5 rounded bg-background border border-border text-muted-foreground">⌘K</kbd>
        </div>
        ...
        <div className="flex items-center gap-1.5">
          ...
          <button className="h-9 w-9 grid place-items-center rounded-lg hover:bg-muted text-muted-foreground"><HelpCircle className="h-4 w-4" /></button>
              ...
              {openNotif && (
                ...
                >
                  ...
                  <div className="max-h-96 overflow-y-auto scrollbar-thin">
                    {notifications.map((n) => (
                      <div key={n.id} className="px-4 py-3 flex gap-3 hover:bg-muted/50 transition border-b border-border last:border-0">
                        <div className={`h-2 w-2 rounded-full mt-2 shrink-0 ${n.unread ? "bg-primary" : "bg-muted-foreground/30"}`} />
          ...
          <div className="ml-2 flex items-center gap-2.5 pl-2 border-l border-border">
            <img src={currentUser.avatar} alt={currentUser.name} className="h-8 w-8 rounded-full border border-border" />
