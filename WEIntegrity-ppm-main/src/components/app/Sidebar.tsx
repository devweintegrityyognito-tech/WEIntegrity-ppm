...
import {
  LayoutDashboard, FolderKanban, Trello, Zap, Users, CalendarCheck, UserCog,
  BarChart3, Bell, Settings, LogOut, Sparkles, Plane, Wallet, UserPlus,
  BookOpen, Puzzle
} from "lucide-react";
  ...
  return (
    <aside className="hidden lg:flex w-64 flex-col bg-sidebar text-sidebar-foreground border-r border-sidebar-border h-screen sticky top-0">
     ...
     <nav className="flex-1 px-3 py-4 space-y-0.5 overflow-y-auto scrollbar-thin">
        ...
        <div className="px-2 text-[10px] font-semibold uppercase tracking-wider text-sidebar-foreground/40 mt-6 mb-2">Account</div>
        <Link to="/plugins" className="flex items-center gap-3 px-3 py-2.5 rounded-lg text-sm text-sidebar-foreground/75 hover:bg-sidebar-accent/60 hover:text-white">
          <Puzzle className="h-[18px] w-[18px]" />
          <span className="font-medium">Plugins</span>
        </Link>
        <Link to="/settings" className="flex items-center gap-3 px-3 py-2.5 rounded-lg text-sm text-sidebar-foreground/75 hover:bg-sidebar-accent/60 hover:text-white">
          <Settings className="h-[18px] w-[18px]" />
          <span className="font-medium">Settings</span>
        </Link>
        <Link to="/login" className="flex items-center gap-3 px-3 py-2.5 rounded-lg text-sm text-sidebar-foreground/75 hover:bg-sidebar-accent/60 hover:text-white">
      ...
      <div className="m-3 p-4 rounded-xl bg-gradient-to-br from-white/5 to-white/0 border border-white/10">
        ...
        <div className="text-[11px] text-sidebar-foreground/60 mt-1">SSO, audit logs & priority support.</div>
        <button className="mt-3 w-full text-xs font-medium bg-white text-sidebar py-1.5 rounded-md hover:bg-white/90 transition">Contact sales</button>
      </div>
    </aside>
  );
}
