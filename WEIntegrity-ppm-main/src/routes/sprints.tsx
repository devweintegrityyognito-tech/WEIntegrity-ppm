...
function SprintsPage() {
  ...
  const donePts = activeTasks.filter((t) => t.status === "Done").reduce((s, t) => s + t.storyPoints, 0);
    ...
    <AppShell>
      <div className="flex items-end justify-between mb-6 flex-wrap gap-3">
        <div>
          ...
          <p className="text-sm text-muted-foreground mt-1">Track sprint goals, capacity and burndown in real time.</p>
        </div>
        <button onClick={() => setOpen(true)} className="h-9 px-3.5 rounded-lg bg-gradient-primary text-primary-foreground text-sm font-medium shadow-elegant">Plan new sprint</button>
      </div>
      ...
      <motion.div initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }}
        ...
        <div className="relative flex flex-wrap items-start justify-between gap-6">
          <div className="max-w-xl">
            <div className="flex items-center gap-2 text-xs text-white/70 uppercase tracking-wider"><Zap className="h-3.5 w-3.5" /> Active sprint</div>
            ...
            <div className="mt-4 flex flex-wrap gap-x-6 gap-y-2 text-xs">
              <span className="inline-flex items-center gap-1.5 text-white/80"><Calendar className="h-3.5 w-3.5" /> {new Date(active.startDate).toLocaleDateString("en-US", { month: "short", day: "numeric" })} → {new Date(active.endDate).toLocaleDateString("en-US", { month: "short", day: "numeric" })}</span>
              <span className="inline-flex items-center gap-1.5 text-white/80"><Target className="h-3.5 w-3.5" /> Capacity {active.capacity} pts</span>
              <span className="inline-flex items-center gap-1.5 text-white/80"><CheckCircle2 className="h-3.5 w-3.5" /> {done} / {activeTasks.length} issues done</span>
            </div>
          </div>
      ...
      <div className="grid grid-cols-1 xl:grid-cols-3 gap-4 mt-6">
        <div className="xl:col-span-2 rounded-xl bg-card border border-border shadow-card p-5">
          ...
          <p className="text-xs text-muted-foreground mt-0.5 mb-3">Ideal vs actual remaining work over the sprint timeline.</p>
          <div className="h-72">
            <ResponsiveContainer>
              <AreaChart data={burndownData} margin={{ top: 6, right: 8, left: -16, bottom: 0 }}>
                ...
                <Area type="monotone" dataKey="actual" name="Actual remaining" stroke="oklch(0.52 0.22 264)" fill="url(#bd)" strokeWidth={2.5} />
                <Line type="monotone" dataKey="ideal" name="Ideal" stroke="oklch(0.7 0.03 260)" strokeDasharray="4 4" strokeWidth={2} dot={false} />
              </AreaChart>
            </ResponsiveContainer>
          </div>
        </div>
        ...
        <div className="rounded-xl bg-card border border-border shadow-card p-5">
          ...
          <div className="space-y-3">
            {sprints.map((s) => (
              <div key={s.id} className="p-3 rounded-lg border border-border hover:bg-muted/30 transition">
                ...
                <div className="mt-2.5 flex items-center justify-between text-[11px]">
                  <span className="text-muted-foreground">{new Date(s.startDate).toLocaleDateString("en-US", { month: "short", day: "numeric" })} – {new Date(s.endDate).toLocaleDateString("en-US", { month: "short", day: "numeric" })}</span>
                  <span className="font-semibold tabular-nums">{s.velocity}/{s.capacity} pts</span>
                </div>
              </div>
            ))}
          </div>
  ...
  return (
    ...
    >
      <label className="block">
        <div className="text-[11px] uppercase tracking-wider text-muted-foreground mb-1">Sprint name *</div>
        <input value={sprintName} onChange={(e) => setName(e.target.value)} required className={inputCls} />
      </label>
      <label className="block">
        <div className="text-[11px] uppercase tracking-wider text-muted-foreground mb-1">Goal</div>
        <textarea value={sprintGoal} onChange={(e) => setGoal(e.target.value)} rows={2} className={inputCls} />
      </label>
      <div className="grid grid-cols-2 gap-3">
        <label className="block">
          <div className="text-[11px] uppercase tracking-wider text-muted-foreground mb-1">Start</div>
        ...
        <label className="block">
          ...
          <select value={status} onChange={(e) => setStatus(e.target.value as SprintStatus)} className={inputCls}>
            {SPRINT_STATUSES.map((s) => <option key={s} value={s}>{s}</option>)}
          </select>
        </label>
        <label className="block">
          <div className="text-[11px] uppercase tracking-wider text-muted-foreground mb-1">Capacity (pts)</div>
          <input type="number" min={0} value={capacity} onChange={(e) => setCapacity(Number(e.target.value))} className={inputCls} />
        </label>
      </div>
