...
function buildCalendar() {
  ...
  const days: { date: number | null; status?: "present" | "leave" | "wfh" | "absent" | "weekend" }[] = [];
    ...
    <AppShell>
      <div className="flex items-end justify-between mb-6 flex-wrap gap-3">
        <div>
          ...
          <p className="text-sm text-muted-foreground mt-1">Track check-ins, leaves and work hours for May 2026.</p>
        </div>
      ...
      <div className="grid grid-cols-1 xl:grid-cols-3 gap-4">
        <motion.div initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }}
          ...
          <div className="relative">
            <div className="text-xs text-white/70 uppercase tracking-wider">Today · Tue, May 19</div>
            ...
            <div className="mt-4 grid grid-cols-3 gap-2 text-center">
              {[
                ...
                <div key={s.l} className="rounded-lg bg-white/10 backdrop-blur border border-white/15 py-2">
        ...
        <div className="xl:col-span-2 rounded-xl bg-card border border-border shadow-card p-5">
          <div className="flex items-center justify-between mb-4">
            ...
            <div className="flex items-center gap-3 text-[11px]">
              {[
                ["Present", "bg-[oklch(0.68_0.16_155)]"], ["WFH", "bg-[oklch(0.65_0.14_230)]"],
                ["Leave", "bg-[oklch(0.78_0.16_75)]"], ["Absent", "bg-destructive"],
              ].map(([l, c]) => (
                <span key={l} className="inline-flex items-center gap-1.5 text-muted-foreground"><span className={`h-2 w-2 rounded-full ${c}`} /> {l}</span>
              ))}
            </div>
          </div>
          ...
          <motion.div initial="h" animate="s" variants={{ s: { transition: { staggerChildren: 0.012 } } }}
            className="grid grid-cols-7 gap-1.5">
            {cal.map((cell, i) => (
              <motion.div key={i} variants={{ h: { opacity: 0, scale: 0.9 }, s: { opacity: 1, scale: 1 } }}
                ...
                {cell.status && <span className={`h-2 w-2 rounded-full ${statusColor[cell.status]}`} />}
              </motion.div>
            ))}
          </motion.div>
        </div>
      ...
      <div className="rounded-xl bg-card border border-border shadow-card mt-6 overflow-hidden">
        <div className="p-5 border-b border-border flex items-center justify-between">
          <div>
            ...
            <p className="text-xs text-muted-foreground mt-0.5">Latest check-ins, breaks and leave events.</p>
          </div>
        ...
        <table className="w-full text-sm">
          ...
          <tbody>
            {[
              { d: "May 19", t: "Office", in: "09:02", out: "—", h: "in progress", s: "Present", tone: "success" as const, Icon: LogIn },
              { d: "May 18", t: "Office", in: "08:58", out: "18:21", h: "9h 23m", s: "Present", tone: "success" as const, Icon: Clock },
              { d: "May 15", t: "Remote", in: "09:12", out: "17:50", h: "8h 38m", s: "WFH", tone: "info" as const, Icon: Coffee },
              { d: "May 14", t: "Remote", in: "08:45", out: "18:10", h: "9h 25m", s: "WFH", tone: "info" as const, Icon: Coffee },
              { d: "May 12", t: "Leave", in: "—", out: "—", h: "—", s: "PTO", tone: "warning" as const, Icon: LogOut },
            ].map((r, i) => (
              <tr key={i} className="border-t border-border hover:bg-muted/30">
                <td className="px-5 py-3 text-sm font-medium">{r.d}</td>
                <td className="py-3 text-xs"><span className="inline-flex items-center gap-1.5 text-muted-foreground"><r.Icon className="h-3.5 w-3.5" /> {r.t}</span></td>
                ...
                <td className="py-3 pr-5"><Badge tone={r.tone} dot>{r.s}</Badge></td>
              </tr>
            ))}
          </tbody>
        </table>
