...
const icons: Record<string, any> = { task: Bell, sprint: Zap, comment: MessageSquare, deploy: Rocket, report: FileBarChart };
    ...
    <AppShell>
      <div className="flex items-end justify-between mb-6 flex-wrap gap-3">
        <div>
          ...
          <p className="text-sm text-muted-foreground mt-1">Stay up to date with sprint, task and deployment activity.</p>
        </div>
      ...
      <div className="flex gap-2 mb-5">
        {["All", "Mentions", "Tasks", "Sprints", "Deploys"].map((t, i) => (
          <button key={t} className={`h-8 px-3.5 rounded-full text-xs font-medium border transition ${i === 0 ? "bg-primary text-primary-foreground border-primary" : "bg-card border-border hover:bg-muted/50"}`}>{t}</button>
        ))}
      </div>
        ...
        {notifications.map((n, idx) => {
          ...
          return (
            <motion.div key={n.id}
              ...
              <div className={`h-10 w-10 rounded-lg grid place-items-center shrink-0 ${n.unread ? "bg-gradient-primary text-white" : "bg-muted text-muted-foreground"}`}>
              ...
              <button className="opacity-0 group-hover:opacity-100 text-xs text-primary font-medium hover:underline transition">View</button>
            </motion.div>
          );
        })}
      </div>
