...
function DefectDetail() {
  ...
  const activity = [
    ...
    ...(defect.resolvedDate ? [{ id: "a4", label: `Marked ${defect.status}`, ts: defect.resolvedDate }] : []),
  ];
      ...
      <div className="flex items-start justify-between flex-wrap gap-3 mb-5">
        <div className="min-w-0">
          ...
          <div className="mt-3 flex flex-wrap items-center gap-2">
            ...
            <Badge tone={statusTone(defect.status === "Resolved" || defect.status === "Closed" ? "Done" : "In Progress")} dot>
              {defect.status}
            </Badge>
          </div>
        </div>
        ...
        <div className="xl:col-span-2 space-y-5">
          ...
          <div className="rounded-xl bg-card border border-border shadow-card p-5">
            ...
            <textarea
              ...
              rows={3}
              placeholder={defect.status === "Resolved" || defect.status === "Closed" ? "Describe the resolution…" : "Pending — defect not yet resolved."}
          ...
          <div className="rounded-xl bg-card border border-border shadow-card p-5">
            <h3 className="font-semibold text-sm mb-3 inline-flex items-center gap-2"><Activity className="h-4 w-4" /> Activity Timeline</h3>
        ...
        <aside className="space-y-3">
          <div className="rounded-xl bg-card border border-border shadow-card p-5">
            ...
            <DetailRow label="Status">
              <select
                value={defect.status}
                onChange={(e) => defectsStore.update(defect.id, { status: e.target.value as DefectStatus })}
            ...
            <DetailRow label="Severity"><Badge tone={priorityTone(defect.severity)}>{defect.severity}</Badge></DetailRow>
            <DetailRow label="Priority"><Badge tone={priorityTone(defect.priority)}>{defect.priority}</Badge></DetailRow>
            ...
            <DetailRow label="Created"><span className="text-xs">{new Date(defect.createdDate).toLocaleDateString()}</span></DetailRow>
            <DetailRow label="Resolved"><span className="text-xs">{defect.resolvedDate ? new Date(defect.resolvedDate).toLocaleDateString() : "—"}</span></DetailRow>
          </div>
          ...
          <div className="rounded-xl bg-card border border-border shadow-card p-5">
            ...
            <div className="space-y-2 text-sm">
              <div>
                <div className="text-[11px] uppercase tracking-wider text-muted-foreground">Related Story</div>
                {story ? (
              ...
              <div>
                <div className="text-[11px] uppercase tracking-wider text-muted-foreground">Related Task</div>
                {relatedTask ? (
                  <Link to="/tasks/$taskId" params={{ taskId: relatedTask.id }} className="text-primary hover:underline">
                    {relatedTask.title}
                  </Link>
                ) : <span className="text-muted-foreground">—</span>}
              </div>
