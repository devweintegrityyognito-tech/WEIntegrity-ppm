...
const cycle = [
  { m: "Apr", v: 41.2 }, { m: "May", v: 42.6 }, { m: "Jun", v: 43.8 },
  { m: "Jul", v: 44.5 }, { m: "Aug", v: 45.9 }, { m: "Sep", v: 47.3 },
];
    ...
    <AppShell>
      <div className="flex items-end justify-between mb-6 flex-wrap gap-3">
        ...
        <div className="flex items-center gap-2">
          <button className="h-9 px-3 rounded-lg border border-border text-sm inline-flex items-center gap-1.5 hover:bg-muted/50"><Download className="h-3.5 w-3.5" /> Export</button>
          <button className="h-9 px-3 rounded-lg bg-gradient-primary text-primary-foreground text-sm font-medium shadow-elegant">Run payroll</button>
        </div>
      </div>

      <div className="grid grid-cols-2 xl:grid-cols-4 gap-4">
        <StatCard label="Gross payout (Sep)" value={47.3} prefix="₹" suffix="L" change={3.1} icon={Wallet} tone="primary" />
        <StatCard label="Net payable" value={39.6} prefix="₹" suffix="L" change={2.8} icon={Receipt} tone="info" />
        <StatCard label="Pending approvals" value={3} change={-25} icon={TrendingUp} tone="warning" />
        <StatCard label="Headcount in scope" value={124} change={1.6} icon={Wallet} tone="success" />
      </div>
          ...
          <div className="h-64">
            <ResponsiveContainer>
              <AreaChart data={cycle}>
                ...
                <Tooltip contentStyle={{ borderRadius: 10, border: "1px solid hsl(var(--border))", background: "hsl(var(--popover))", fontSize: 12 }} />
                <Area type="monotone" dataKey="v" stroke="oklch(0.55 0.18 265)" strokeWidth={2} fill="url(#py)" />
              </AreaChart>
            </ResponsiveContainer>
          </div>
        </div>
                ...
                <tr key={u.id} className="border-t border-border hover:bg-muted/30 transition">
                  <td className="px-5 py-3">
                    <div className="flex items-center gap-2.5">
                      <img src={u.avatar} className="h-7 w-7 rounded-full border border-border" alt="" />
                      <span className="font-medium">{u.name}</span>
                    </div>
                  </td>
                  <td className="px-5 py-3 text-muted-foreground">Sep 2026</td>
                  <td className="px-5 py-3 tabular-nums">₹ {(85000 + i * 6500).toLocaleString()}</td>
                  <td className="px-5 py-3 tabular-nums">₹ {(72000 + i * 5400).toLocaleString()}</td>
                  <td className="px-5 py-3"><Badge tone={i < 4 ? "success" : "warning"} dot>{i < 4 ? "Processed" : "Pending"}</Badge></td>
                </tr>
              ))}
            </tbody>
          </table>
