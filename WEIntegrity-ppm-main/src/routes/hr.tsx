...
const headcount = [
  { m: "Apr", v: 112 }, { m: "May", v: 116 }, { m: "Jun", v: 119 },
  { m: "Jul", v: 121 }, { m: "Aug", v: 123 }, { m: "Sep", v: 124 },
];
const attendanceTrend = [
  { d: "Mon", present: 118, leave: 4 }, { d: "Tue", present: 120, leave: 3 },
  { d: "Wed", present: 117, leave: 5 }, { d: "Thu", present: 119, leave: 4 },
  { d: "Fri", present: 115, leave: 7 }, { d: "Sat", present: 64, leave: 2 },
];
    ...
    <AppShell>
      <div className="flex items-end justify-between mb-6 flex-wrap gap-3">
        <div>
          ...
          <p className="text-sm text-muted-foreground mt-1">People metrics, attendance and workforce health.</p>
        </div>
      ...
      >
        ...
        <StatCard label="Present today" value={114} change={2.4} icon={CalendarCheck} tone="success" />
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-4 mt-6">
        <div className="lg:col-span-2 rounded-xl bg-card border border-border shadow-card p-5">
          <div className="flex items-center justify-between mb-4">
            ...
            <Badge tone="success"><TrendingUp className="h-3 w-3 mr-1 inline" /> 96.4% avg</Badge>
          </div>
          <div className="h-64">
            <ResponsiveContainer>
          ...
          <div className="h-64">
            <ResponsiveContainer>
              <AreaChart data={headcount}>
                ...
                <Tooltip contentStyle={{ borderRadius: 10, border: "1px solid hsl(var(--border))", background: "hsl(var(--popover))", fontSize: 12 }} />
                <Area type="monotone" dataKey="v" stroke="oklch(0.55 0.18 265)" strokeWidth={2} fill="url(#hc)" />
              </AreaChart>
            </ResponsiveContainer>
          </div>
        </div>
        ...
        <div className="divide-y divide-border">
          {users.slice(0, 5).map((u) => (
            <div key={u.id} className="px-5 py-3 flex items-center gap-3">
              <img src={u.avatar} className="h-9 w-9 rounded-full border border-border" alt={u.name} />
              <div className="flex-1 min-w-0">
                <div className="text-sm font-medium truncate">{u.name}</div>
                <div className="text-xs text-muted-foreground truncate">{u.title} · {u.team}</div>
              </div>
