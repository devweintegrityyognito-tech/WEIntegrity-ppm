    ...
    <AppShell>
      <div className="flex items-end justify-between mb-6 flex-wrap gap-3">
        <div>
          ...
          <p className="text-sm text-muted-foreground mt-1">Drag through columns to reorder, click chevrons to advance status.</p>
        </div>
        <div className="flex items-center gap-2">
          <div className="flex -space-x-2">
            {["u-2", "u-4", "u-5", "u-8"].map((id) => <img key={id} src={userById(id).avatar} className="h-8 w-8 rounded-full border-2 border-background" />)}
          </div>
          <button className="h-9 px-3.5 rounded-lg bg-gradient-primary text-primary-foreground text-sm font-medium inline-flex items-center gap-1.5 shadow-elegant"><Plus className="h-4 w-4" /> Add issue</button>
        </div>
      </div>
          ...
          <div key={col.id} className="w-80 shrink-0">
            <div className="flex items-center justify-between mb-3 px-1">
              <div className="flex items-center gap-2">
                ...
                <span className="text-xs text-muted-foreground bg-muted px-1.5 py-0.5 rounded-full tabular-nums">{byCol[col.id].length}</span>
              </div>
              <button className="text-muted-foreground hover:text-foreground"><Plus className="h-4 w-4" /></button>
            </div>
            <Reorder.Group axis="y" values={byCol[col.id]} onReorder={(reordered) => {
              setItems((all) => {
                    ...
                    <Reorder.Item key={t.id} value={t} className="list-none">
                      ...
                      >
                        ...
                        <div className="mt-2 flex items-center gap-1.5 flex-wrap">
                          {t.tags.map((tag) => <span key={tag} className="text-[10px] px-1.5 py-0.5 rounded bg-muted text-muted-foreground">#{tag}</span>)}
                        </div>
                        <div className="mt-3 pt-3 border-t border-border flex items-center justify-between">
                          <div className="flex items-center gap-3 text-xs text-muted-foreground">
                            <span className="font-mono">{t.key}</span>
                            <span className="inline-flex items-center gap-1"><MessageSquare className="h-3 w-3" /> 4</span>
                            <span className="inline-flex items-center gap-1"><Paperclip className="h-3 w-3" /> 2</span>
                          </div>
                          <div className="flex items-center gap-1.5">
                            <button onClick={() => move(t, -1)} className="text-xs px-1 text-muted-foreground hover:text-foreground">‹</button>
                            <img src={a.avatar} title={a.name} className="h-6 w-6 rounded-full border border-border" />
                            <button onClick={() => move(t, 1)} className="text-xs px-1 text-muted-foreground hover:text-foreground">›</button>
                          </div>
                        </div>
                        <div className="mt-2 flex items-center justify-between text-[11px] text-muted-foreground">
                          <span className="inline-flex items-center gap-1"><Calendar className="h-3 w-3" /> {new Date(t.dueDate).toLocaleDateString("en-US", { month: "short", day: "numeric" })}</span>
