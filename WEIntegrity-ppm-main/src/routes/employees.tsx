    ...
    <AppShell>
      <div className="flex items-end justify-between mb-6 flex-wrap gap-3">
        <div>
          ...
          <p className="text-sm text-muted-foreground mt-1">Workforce directory and lifecycle management.</p>
        </div>
      ...
      <div className="rounded-xl bg-card border border-border shadow-card overflow-hidden">
        <div className="px-4 py-3 border-b border-border flex items-center gap-2 flex-wrap">
          <div className="relative flex-1 min-w-[240px]">
            ...
            <input placeholder="Search by name, team, role…" className="w-full h-9 pl-9 pr-3 rounded-lg bg-muted/50 border border-transparent focus:border-ring focus:bg-card text-sm outline-none transition" />
          </div>
                ...
                >
                  <td className="px-5 py-3">
                    <div className="flex items-center gap-2.5">
                      <img src={u.avatar} className="h-8 w-8 rounded-full border border-border" alt={u.name} />
                      <div>
                  ...
                  <td className="px-5 py-3"><Badge tone="primary">{u.role}</Badge></td>
                  <td className="px-5 py-3">
                    <Badge tone={u.status === "online" ? "success" : u.status === "away" ? "warning" : "muted"} dot>{u.status}</Badge>
                  </td>
                  ...
                  <td className="px-2 py-3 text-muted-foreground"><MoreHorizontal className="h-4 w-4" /></td>
                </motion.tr>
              ))}
            </tbody>
          </table>
