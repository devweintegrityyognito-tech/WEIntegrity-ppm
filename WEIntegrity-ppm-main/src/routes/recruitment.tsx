    ...
    <AppShell>
        ...
        {stages.map((s) => (
          <div key={s.key} className="rounded-xl bg-muted/30 border border-border p-3">
            <div className="flex items-center justify-between mb-3 px-1">
              <div className="text-xs font-semibold uppercase tracking-wider text-foreground/70">{s.key}</div>
              ...
              {(candidates[s.key] || []).map((c, i) => (
                ...
                >
                  ...
                  <div className="flex items-center justify-between mt-2">
                    <span className="text-[11px] text-muted-foreground inline-flex items-center gap-1"><MapPin className="h-3 w-3" />{c.loc}</span>
      ...
      <div className="rounded-xl bg-card border border-border shadow-card overflow-hidden">
        ...
        <div className="divide-y divide-border">
          {positions.map((p) => (
            <div key={p.title} className="px-5 py-4 flex items-center gap-4 hover:bg-muted/30 transition">
              <div className="h-9 w-9 rounded-lg bg-gradient-primary/10 text-primary grid place-items-center">
                <Briefcase className="h-4 w-4" />
              </div>
              <div className="flex-1 min-w-0">
                <div className="text-sm font-semibold">{p.title}</div>
                <div className="text-xs text-muted-foreground">{p.team} · {p.loc}</div>
              </div>
              <div className="text-xs text-muted-foreground tabular-nums">{p.apps} applicants</div>
              <button className="text-xs font-medium text-primary hover:underline">View pipeline</button>
            </div>
          ))}
        </div>
      </div>
