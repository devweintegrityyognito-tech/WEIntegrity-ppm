...
export const Route = createFileRoute("/permissions/edit/$permissionId")({
...
function EditPermissionPage() {
  ...
  const [status, setStatus] = useState<PermissionStatus>("Active");
  ...
  useEffect(() => {
    async function loadPermission() {
      try {
        ...
        setModuleName(permission.moduleName || "");
        setResourceName(permission.resourceName || "");
        setAction(permission.action || "Read");
        setDescription(permission.description || "");
        setStatus(permission.status || "Active");
        setSystemPermission(permission.systemPermission || false);

        setCreatedBy(permission.createdBy || "");
        setCreatedDate(permission.createdDate || "");
      } catch (err) {
        console.error(err);
  ...
  async function handleUpdate(e: React.FormEvent) {
    ...
    try {
      await permissionsStore.update(permissionId, {
      ...
      <form onSubmit={handleUpdate} className="space-y-5">
        <div className="rounded-xl bg-card border border-border shadow-card overflow-hidden">
          <div className="flex items-center gap-3 px-6 py-4 border-b border-border bg-muted/20">
            ...
            <div>
              <div className="font-semibold leading-tight">Update Permission</div>

              <div className="text-xs text-muted-foreground">Update permission information</div>
            </div>
          </div>
