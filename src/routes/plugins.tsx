  ...
  return (
    <div className="max-w-4xl mx-auto">
      <div className="flex items-center justify-between mb-2">
        ...
        <span className="text-sm text-muted-foreground">Yognito Marketplace</span>
      </div>

      <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-8">
        <div className="rounded-xl border border-border bg-card p-6 shadow-card">
          <div className="text-sm text-muted-foreground">Installed Modules</div>

          <div className="text-3xl font-bold mt-2">{getInstalledPlugins().length}</div>
        </div>

        <div className="rounded-xl border border-border bg-card p-6 shadow-card">
          <div className="text-sm text-muted-foreground">Available Modules</div>

          <div className="text-3xl font-bold mt-2">{availablePlugins.length}</div>
        </div>
      </div>
      ...
      <div className="space-y-6">
        ...
        {availablePlugins
          .filter((plugin) => plugin.name.toLowerCase().includes(search.toLowerCase()))
          .filter(
            (plugin) => selectedCategory === "All Modules" || plugin.category === selectedCategory,
          )
          .sort((a, b) => {
            if (sortBy === "Installed") {
              return Number(isPluginInstalled(b.id)) - Number(isPluginInstalled(a.id));
            }
                ...
                {installed && (
                  <div className="mt-5 p-4 rounded-lg bg-green-50 border border-green-200">
                    ...
                    <div className="text-sm text-muted-foreground mt-1">
                      Use the Scope Switcher (Grid icon beside Notifications) to switch between
                    ...
                    <div className="mt-4 flex gap-3 flex-wrap">
                      ...
                      <Link to="/dashboard" className="px-5 py-2 rounded-lg border border-border">
                        Home
                      </Link>
