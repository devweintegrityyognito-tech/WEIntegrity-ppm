    ...
    <AppShell>
      <div className="flex items-end justify-between mb-6 flex-wrap gap-3">
        <div>
          ...
          <p className="text-sm text-muted-foreground mt-1">
            {users.length} members across 8 teams
          </p>
        </div>
      ...
      <div className="relative max-w-sm mb-5">
        ...
        <input
          ...
          className="w-full h-9 pl-9 pr-3 rounded-lg bg-card border border-border text-sm outline-none focus:border-ring"
        />
      </div>
      ...
      <motion.div
        initial="h"
        animate="s"
        variants={{ s: { transition: { staggerChildren: 0.04 } } }}
          ...
          >
            ...
            <div className="flex items-center gap-3">
              <div className="relative">
                ...
                <span
                  className={`absolute bottom-0 right-0 h-3.5 w-3.5 rounded-full border-2 border-card ${u.status === "online" ? "bg-[oklch(0.68_0.16_155)]" : u.status === "away" ? "bg-[oklch(0.78_0.16_75)]" : "bg-muted-foreground/50"}`}
                />
              </div>
              <div className="min-w-0">
                ...
                <div className="mt-1.5">
                  <Badge
                    tone={u.role === "Admin" ? "primary" : u.role === "Manager" ? "info" : "muted"}
                  >
                    {u.role}
                  </Badge>
                </div>
            ...
            <div className="mt-4 pt-4 border-t border-border flex items-center justify-between text-xs">
              <span className="text-muted-foreground">
                Team · <span className="text-foreground font-medium">{u.team}</span>
              </span>
              <a
                href={`mailto:${u.email}`}
