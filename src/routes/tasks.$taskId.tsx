...
function TaskDetail() {
  ...
  const task = useMemo(() => allTasks.find((t) => t._id === taskId), [allTasks, taskId]);
          ...
          <div className="rounded-xl bg-card border border-border shadow-card p-5">
            ...
            <DetailRow label="Status">
              <select
                value={task.status}
                onChange={(e) =>
                  scrumTasksStore.update(task.id, { status: e.target.value as TaskStatus })
                }
            ...
            <DetailRow label="Estimated">
              <span className="text-xs tabular-nums">{task.estimatedHours}h</span>
            </DetailRow>
            <DetailRow label="Actual">
              <span className="text-xs tabular-nums">{task.actualHours}h</span>
            </DetailRow>
